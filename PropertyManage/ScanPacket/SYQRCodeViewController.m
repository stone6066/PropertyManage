...
@property (nonatomic,copy)NSString *webUrlStr;//非本站链接url
...
- (void)initTitleView
{
    ...
    bgView.backgroundColor = topSearchBgdColor;//[UIColor colorWithRed:219.0/255 green:41.0/255 blue:45.0/255 alpha:1.0];
    [self.view addSubview:bgView];
    
    UILabel *titleLab = [[UILabel alloc] initWithFrame:CGRectMake((kDeviceWidth - 80) / 2.0, 28, 80, 20)];
...
//    [btn addTarget:self action:@selector(cancleSYQRCodeReading) forControlEvents:UIControlEventTouchUpInside];
    ...
    [btn_1 setFrame:CGRectMake(0, 0, 120, 50)];
    
    [btn_1 addTarget:self action:@selector(cancleSYQRCodeReading) forControlEvents:UIControlEventTouchUpInside];
...
//            _webUrlStr=obj.stringValue;
//            if ([obj.stringValue containsString:MainToken])
//            {
//                if (self.SYQRCodeSuncessBlock) {
//                    self.SYQRCodeSuncessBlock(self,obj.stringValue);
//                }
...
//            else
//            {
//                NSString *messageStr=[NSString stringWithFormat:@"%@%@",@"不是本站链接，是否打开?",obj.stringValue];
//                UIAlertView *myalert=[[UIAlertView alloc]initWithTitle:@"提示" message:messageStr delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"打开链接", nil];
//                [myalert show];
//            }
        }
        else
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    switch (buttonIndex) {
        ...
        case 1:{//打开非本站链接
            ...
            NSLog(@"%@",_webUrlStr);
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:_webUrlStr]];
        }break;
